...
**File:** `src\server\search\confidenceCalibrator.ts`
...
## Signature
```typescript
function calibrateScore(crossEncoderScore: number,          // existing finalScore (0–100)
    evidenceScore: number,              // existing evidenceConfidence (0–100)
    candidate: UnifiedCandidate,
    matches: RequirementMatch[],
    profile: RequirementProfile): CalibratedScore
```
