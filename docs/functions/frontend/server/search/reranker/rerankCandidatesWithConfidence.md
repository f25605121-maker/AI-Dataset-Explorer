# rerankCandidatesWithConfidence

**File:** `src\server\search\reranker.ts`
...
## Signature
```typescript
function rerankCandidatesWithConfidence(candidates: (NormalizedSearchResult | UnifiedCandidate)[],
    querySchema: ResearchQuerySchema | StructuredQueryUnderstanding,
    topK = 50): RerankResult
```
