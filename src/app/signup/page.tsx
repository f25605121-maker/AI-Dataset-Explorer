            ...
            <div className="relative w-full max-w-md z-10 space-y-6">
                ...
                <div className="rounded-3xl border border-strong glass-card p-7 sm:p-8 shadow-2xl space-y-6">
                    <div className="text-center space-y-1">
                        <h1 className="text-2xl font-black text-primary">Create an Account</h1>
                        <p className="text-xs text-muted">Join AI researchers, engineers, and builders</p>
                    </div>
                    ...
                    <form
                        ...
                        onSubmit={async (e) => {
                            ...
                            try {
                                ...
                                if (res.ok) {
                                    await signIn("credentials", { email, password, callbackUrl: "/explore" });
                                } else {
                                    const err = await res.json().catch(() => ({ message: "Registration failed" }));
                                    setError(err.message || "Registration failed");
                                }
                            } catch {
                                setError("Unable to register. Please check your network connection.");
                            } finally {
                                setIsLoading(false);
                            }
                        }}
                    >
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-wider text-muted mb-2" htmlFor="name">
                                Full Name
                            </label>
                            <input
                                id="name"
                        ...
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-wider text-muted mb-2" htmlFor="password">
                                Password
                            </label>
                ...
                <p className="text-center text-xs text-muted">
                    ...
                    <Link href="/login" className="font-bold text-primary hover:text-accent-to transition underline-offset-4 hover:underline">
                        Sign in
                    </Link>
                </p>
            </div>
