...
function shouldRejectModel(model: Partial<NormalizedModel>, qu: QueryUnderstanding): string | null {
    ...
    // Vision/imaging project + irrelevant text/speech model (fixed regex: removed generic 'detection')
    const isImagingQuery = /ct|mri|x.ray|image|radiograph|ultrasound|mammograph|fundus|dermoscopy|visual\s*detection|object\s*detection/.test(queryTask + ' ' + queryModality);
...
function scoreModelTask(model: Partial<NormalizedModel>, qu: QueryUnderstanding): number {
    ...
    const taskFamilies: [string[], string[]][] = [
        [['reconstruction', 'reconstruct', 'k-space'], ['image-to-image', 'reconstruction', 'image-reconstruction', 'unconditional-image-generation']],
        [['velocity_estimation', 'velocity estimation', 'regression', 'flow estimation'], ['regression', 'image-to-image', 'velocity-estimation']],
        [['registration'], ['image-registration', 'registration']],
        ...
        [['classification', 'detection', 'seizure detection', 'diagnosis'], ['image-classification', 'text-classification', 'audio-classification', 'time-series-classification', 'classification']],
    ...
    // Partial keyword match & target match
    const taskWords = queryTask.split(/\s+/).filter(w => w.length > 3);
    const matched = taskWords.filter(w => modelText.includes(w)).length;
    if (taskWords.length > 0 && matched > 0) {
        let score = Math.round((matched / taskWords.length) * 70);
        if (queryTarget && modelText.includes(queryTarget)) score += 15;
        return Math.min(90, score);
    }

    if (queryTarget && modelText.includes(queryTarget)) return 60;
...
function scoreModelModality(model: Partial<NormalizedModel>, qu: QueryUnderstanding): number {
    ...
    // Includes
    if (modelModality.includes(queryModality) || queryModality.includes(modelModality)) return 85;

    // Signal / EEG / Time-series family
    const signalMods = ['eeg', 'ecg', 'time-series', 'signal', 'sensor', 'intracranial eeg', 'ieeg'];
    const queryIsSignal = signalMods.some(m => queryModality.includes(m));
    const modelIsSignal = signalMods.some(m => modelModality.includes(m) || modelText.includes(m));
    if (queryIsSignal && modelIsSignal) return 90;
    ...
    const imagingMods = ['ct', 'mri', 'x-ray', 'image', 'video'];
    const nonImagingMods = ['tabular', 'text', 'audio', 'time-series', 'eeg'];
    if (imagingMods.some(m => queryModality.includes(m)) && nonImagingMods.some(m => modelModality === m)) return 0;
...
function scoreModelArchitecture(model: Partial<NormalizedModel>, qu: QueryUnderstanding): number {
    ...
    const archTaskAlignments: [RegExp, string[]][] = [
        [/4dflow|flow.?net|varnet|modl|k-space|pinn/i, ['reconstruction', 'velocity_estimation', 'velocity estimation', 'regression']],
        [/yolo|detr|faster.?rcnn|efficientdet|rtdetr/i, ['detection', 'object detection']],
        [/unet|segformer|sam|mask.?rcnn|panoptic/i, ['segmentation', 'instance segmentation', 'semantic segmentation', 'reconstruction']],
