...
export function calculateOrderAmount(
    ...
    const plan = SERVER_PRICING_CATALOG[planId.toLowerCase()];
    if (!plan) {
        throw new Error(`Invalid plan identifier: ${planId}`);
    }

    const baseAmountCents =
        billingInterval === 'yearly' ? plan.yearlyPriceCents : plan.monthlyPriceCents;

    let discountPercent = 0;
    if (couponCode) {
        const cleanCoupon = couponCode.toUpperCase().trim();
        discountPercent = VALID_DISCOUNT_CODES[cleanCoupon] || 0;
    }

    const discountAmountCents = Math.round((baseAmountCents * discountPercent) / 100);
